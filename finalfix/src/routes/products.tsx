...
function ProductsPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-14">
      ...
      <div className="mt-8">
        {loading ? <p className="py-16 text-center text-sm text-subtle">جاري تحميل المنتجات...</p> : error ? <p className="py-16 text-center text-sm text-red-600">{error}</p> : list.length ? <ProductGrid products={list} /> : <p className="py-16 text-center text-sm text-subtle">{t("shop.empty")}</p>}
      </div>
    </div>
  );
}
